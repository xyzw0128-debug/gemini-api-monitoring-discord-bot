...
- **모델별 병렬·스태거 실행**: 같은 모델에 연결된 서로 다른 키는 `model_key_parallelism` 한도 안에서 함께 점검하고, 다음 모델 전에는 `probe_stagger_sec`만큼 대기합니다. 작업은 하나의 잠금으로 직렬화되며, 동일한 작업(예: "전체 재확인")이 이미 진행 중이면 중복 실행되지 않습니다.
...
- **OpenClaw 실사용 감지 (선택 기능)**: 외부 프로세스의 로그를 구독해 `provider=google` 관련 429·과부하·타임아웃을 감지하면, 그 자체로 상태를 바꾸지 않고 해당 모델에 연결된 설정된 수의 대표 키를 재확인합니다. 최종 상태는 항상 직접 프로브 결과로만 결정됩니다.
...
전체 재확인은 같은 모델에 연결된 모든 키를 먼저 병렬 점검한 뒤 다음 모델로 넘어갑니다. 모델별 제한 여부를 키 간에 비교하기 쉬우며, 동시 점검 수는 `model_key_parallelism`으로 제한합니다.
...
다른 모델 그룹 및 다른 프로브 작업은 하나의 락으로 직렬화됩니다. 같은 모델 안에서만 키별 요청을 병렬 실행하고, 다음 모델 그룹 전에는 `probe_stagger_sec`만큼 대기합니다.
...
| `/test model name:google/<모델명>` | 자동완성 목록에서 모델을 골라, 연결된 모든 키를 확인합니다. |
| `/test key id:<이름>` | 자동완성 목록에서 키 ID를 골라, 연결된 모든 모델을 확인합니다. |
...
### `NameError: probe_key_limit is not defined` 또는 OpenClaw 감지 뒤 `TypeError`가 남

이 오류는 `probe_key_limit` 설정값을 읽지 않거나, OpenClaw 재확인 요청의 키 수 제한을 저장소에 전달하지 못했던 이전 구현의 문제입니다. 현재 버전은 `openclaw_observer.probe_key_limit`을 기본값 `1`로 읽고 최소 `1`인지 검증하며, 감지된 각 모델에서 등록 순서 기준으로 지정한 수만큼의 키를 재확인합니다. 이 오류가 계속되면 실행 중인 소스가 이 수정본인지 확인한 뒤 서비스를 재시작하세요.
